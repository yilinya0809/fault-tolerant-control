...
vz_init = 1.43950854
Fp_init = 82.85
theta_init = np.deg2rad(1.81)

""" Get trim """
X_trim, U_trim = plant.get_trim(fixed={"h": 10, "VT": 0})
...
# Q = ca.diagcat(30, 30, 30)  # Gain matrix for X
# R = ca.diagcat(0.01, 0.1, 20000)  # Gain matrix for U
Q = ca.diagcat(100, 10, 1)
R = ca.diagcat(0.001, 0, 1000)


# Q = ca.diagcat(100, 100, 100)
# R = ca.diagcat(0.01, 0.1, 100)

Xdot = plant.derivq(states, controls, q=0.0)
...
lbx[n_states * (N + 1) + 2 :: n_controls] = -np.deg2rad(30)  # theta min
ubx[n_states * (N + 1) + 2 :: n_controls] = np.deg2rad(30)  # theta min
...
control_init = ca.DM([plant.m * plant.g, Fp_init, theta_init])

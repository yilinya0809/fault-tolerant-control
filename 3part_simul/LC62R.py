...
class LC62R(fym.BaseEnv):
    ...
    def __init__(self, pos0, vel0, quat0, omega0, is_lon=False):
        super().__init__()
        self.pos = fym.BaseSystem(pos0)
        self.vel = fym.BaseSystem(vel0)
        self.quat = fym.BaseSystem(quat0)
        self.omega = fym.BaseSystem(omega0)
        self.is_lon = is_lon
    ...
    def deriv_lin(self, pos, vel, quat, ang, omega, FM):
        F, M = FM[0:3], FM[3:]
        dcm = quat2dcm(quat)
        phi, theta, _ = np.ravel(ang)
        p, q, r = np.ravel(omega)

        """ dynamics """
        dpos = dcm.T @ vel
        dvel = F / self.m - np.cross(omega, vel, axis=0)
        dphi = p + sin(phi) * np.tan(theta) * q + cos(phi) * np.tan(theta) * r
        dtheta = cos(phi) * q - sin(phi) * r
        dpsi = (sin(phi) * q + cos(phi) * r) / cos(theta)
        dang = np.vstack((dphi, dtheta, dpsi))
        domega = self.Jinv @ (M - np.cross(omega, self.J @ omega, axis=0))
...
if __name__ == "__main__":
    pos0 = np.vstack((0.0, 0.0, 0.0))
    vel0 = np.vstack((0, 0, 0))
    quat0 = np.vstack((1, 0, 0, 0))
    omega0 = np.zeros((3, 1))
    system = LC62R(pos0, vel0, quat0, omega0)

...
def linear_interpolate(x, xp, fp, fill_value="extrapolate"):
    ...
    if fill_value == "extrapolate":
        below = x < xp[0]
        above = x > xp[-1]
        y = cp.where(below, fp[0] + (x - xp[0]) * (fp[1] - fp[0]) / (xp[1] - xp[0]), y)
        y = cp.where(
            above, fp[-1] + (x - xp[-1]) * (fp[-1] - fp[-2]) / (xp[-1] - xp[-2]), y
        )

    return y


def polyval_cvxp(coeffs, x):
    degree = len(coeffs) - 1
    poly_expr = cp.Constant(0)
    for i in range(degree + 1):
        poly_expr += coeffs[i] * cp.power(x, degree - i)
    return poly_expr
...
class LC62_corridor(fym.BaseEnv):
    ...
    def __init__(self, env_config={}):
        ...
        # self.x_trims, self.u_trims_fixed = self.get_trim_fixed(fixed={"h": 10, "VT": 45})
    ...
    def B_VTOL(self, rcmds, omega):
        ...
        th = polyval_cvxp(self.tables["th_r"], rcmds) * self.g / 1000
        tq = polyval_cvxp(self.tables["tq_r"], rcmds)
        Fx = Fy = 0
    ...
    def B_Pusher(self, pcmds):
        ...
        th_p = linear_interpolate(
            pcmds, self.tables["cmd"], self.tables["th_p"], fill_value="extrapolate"
        )
        tq_p = linear_interpolate(
            pcmds, self.tables["cmd"], self.tables["tq_p"], fill_value="extrapolate"
        )
        # th = th_p(pcmds)
        # tq = tq_p(pcmds)
        Fx = th[0] + th[1]
    ...
    def get_corr(
        ...
        for i in range(n):
            for j in range(m):
                theta = theta_range[j]
                vel = VT_range[i]
                fixed = (height, vel, theta)

                z = cp.Variable(8)
                cost_expr = self._cost_fixed(z, fixed)
                objective = cp.Minimize(cost_expr)

                problem = cp.Problem(objective)
                problem.solve()
                cost[i][j] = problem.value
                # result = scipy.optimize.minimize(
                #     self._cost_fixed,
                #     z0,
                #     args=(fixed,),
    ...
    def _cost_fixed(self, z, fixed):
        ...
        pos_trim = cp.vstack((0, 0, -h))
        vel_trim = cp.vstack((vel * cos(theta), 0, vel * sin(theta)))
        quat_trim = cp.vstack(angle2quat(0, theta, 0))
        omega_trim = cp.vstack((0, 0, 0))
        rcmds = cp.vstack((rotor1, rotor2, rotor3, rotor4, rotor5, rotor6))
        pcmds = cp.vstack((pusher1, pusher2))
        dels = cp.vstack((0, 0, 0))
...
if __name__ == "__main__":
    ...
    height = 50
    # grid = {"VT": np.arange(0, 45.1, 0.1), "theta": np.deg2rad(np.arange(-30, 30, 0.2))}

...
class LC62_corridor(fym.BaseEnv):
    ...
    def get_corr(
        ...
        if np.linalg.norm(result.fun) < eps:
            self.VT, self.alp, self.r1, self.r2, self.r3, self.r4, self.r5, self.r6, self.p1, self.p2 = result.x
            z0={
                "VT": self.VT,
                "alp": self.alp,
                "rotor1": self.r1,
            ...
            success.append(1)
            vel.append(self.VT)
            theta.append(np.rad2deg(self.alp))
            print(f"vel: {vel[-1]:.1f}, theta: {theta[-1]:.1f}, success")
        else:
            success.append(0)
            print(f"vel: {vel[-1]:.1f}, theta: {theta[-1]:.1f}, cost: {cost[-1]:.3f}")

                    
        Trst_corr = vel, theta, cost, success
        return Trst_corr

    def _cost_fixed(self, z, fixed):
        h, Vz_max = fixed
        VT, alp, rotor1, rotor2, rotor3, rotor4, rotor5, rotor6, pusher1, pusher2 = z
        pos_trim = np.vstack((0, 0, -h))
        ...
        x1 = (np.sign(dpos[0]) - 1) * dpos[0]
        # x1 = dpos[0] - vel * cos(theta)
        x2 = dpos[1]
        x3 = (np.sign(dpos[2]**2 - Vz_max * dpos[2]) + 1) * (dpos[2]**2 - Vz_max * dpos[2])
        ...
        dxs = np.vstack((x1, x2, x3, x4, x5, x6, x7))
        weight = np.diag([1, 1, 1, 1, 1, 1, 1000, 1000, 1000])

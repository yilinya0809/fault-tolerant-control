...
class MPC_Corr:
    def __init__(self, env):
        ...
        self.z_init, vx_init, vz_init, theta_init, _ = env.observation()
        X_trim, U_trim = self.plant.get_trim(fixed={"h": 50, "VT": 40})
        _, self.z_target, _, _ = X_trim.ravel()
        # Fr_target, Fp_target, theta_target = U_trim.ravel()
        self.control_init = ca.DM([-self.plant.m * self.plant.g, 0, theta_init])

        self.state_init = ca.DM([self.z_init, vx_init, vz_init])
        self.set_ref(0, env.clock.max_t, VT_corr[0], VT_corr[-1])

        # self.state_target = ca.DM([self.z_target, vx_target, vz_target])
        theta_target = theta_corr[0, -1]
        self.control_target = ca.DM([0, 0, theta_target])
    ...
    def solve_mpc(self, obs):
        ...
        for k in range(N):
            ...
            cost_fn = cost_fn + st_err.T @ Q @ st_err
            # + con_err.T @ R @ con_err
            st_next = X[:, k + 1]
            k1 = f(st, con)

...
class MPC:
    def __init__(self, env):
        ...
        # self.state_init = ca.DM([z_init, vx_init, vz_init, 0, 0, 0])
    ...
    def solve_mpc(self, obs):
        ...
        Q = ca.diagcat(100, 100, 20)
        # Q = ca.diagcat(100, 100, 20, 1, 1, 1)
        R = 0.005 * ca.diagcat(1, 0, 100000)
        S = 0.002 * ca.diagcat(1, 1, 0)
        ...
        for k in range(N - 1):
            con = U[:, k]
            con_next = U[:, k + 1]
            con_diff = con_next - con
            cost_fn = cost_fn + con_diff.T @ S @ con_diff
            g = ca.vertcat(g, con_diff)
...
#         # self.state_init = ca.DM([z_init, vx_init, vz_init])
#         self.state_init = ca.DM([z_init, vx_init, vz_init, 0, 0, 0])
#         self.state_target = ca.DM([self.z_target, vx_target, vz_target, 0, 0, 0])
#         self.control_target = ca.DM([Fr_target, Fp_target, theta_target])
#         self.n_states = self.state_init.numel()
#         self.n_controls = self.control_init.numel()
#         self.args = self.constraints()
...
#         lbx[2 : n_states * (N + 1) : n_states] = -ca.inf  # Vz min
#         ubx[2 : n_states * (N + 1) : n_states] = ca.inf  # Vz max
#         lbx[3 : n_states * (N + 1) : n_states] = -20  # Fr_dot min
#         ubx[3 : n_states * (N + 1) : n_states] = 20 # Fr_dot max
#         lbx[4 : n_states * (N + 1) : n_states] = -20  # Fp_dot min
#         ubx[4 : n_states * (N + 1) : n_states] = 20  # Fp_dot max
#         lbx[5 : n_states * (N + 1) : n_states] = -ca.inf  # theta_dot min
#         ubx[5 : n_states * (N + 1) : n_states] = ca.inf  # theta_dot max
...
#         z, vx, vz, theta, q = obs
#         Fr, Fp, _ = np.ravel(self.control_init)
#         state_init = ca.DM([z, vx, vz, 0, 0, 0])
#         control_init = ca.DM([Fr, Fp, theta])

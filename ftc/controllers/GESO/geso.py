...
class GESOController(fym.BaseEnv):
    def __init__(self, env):
        ...
        # Observer state = m*zdot, Jw
        self.num_y = 4
        self.obsv = fym.BaseSystem(np.zeros((self.num_y*2, 1)))
        l0 = 1
        l1 = 1
        self.L = np.vstack((l0 * np.eye(self.num_y), l1 * np.eye(self.num_y)))
    ...
    def get_control(self, t, env):
        ...
        dhat = self.obsv.state[self.num_y:]
        nui_E = -dhat
        nui = nui_N + nui_E
    ...
    def set_dot(self, t, env):
        pos, vel, quat, omega = env.plant.observe_list()
        y = np.vstack((env.plant.m * vel[2], env.plant.J @ omega))
        l = np.size(y)

        A = np.eye(2*l, 2*l, l)
        B = np.zeros((l * 2, l))
        B[0, 0] = quat2dcm(quat).T[2, 2]
        B[1:l, 1:l] = np.eye(l - 1)
